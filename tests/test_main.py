...
def test_web_to_database():
    ...
    main.create_university_info(cursor)
    main.web_to_database(cursor, values)
...
def test_create_unemployment_data():
    ...
    conn, cursor = main.open_db("project_db.sqlite")
    cursor.execute('DROP TABLE IF EXISTS Employment_Data')
    main.create_employment_data(cursor)
    main.excel_to_database(excel_data_path, cursor)
...
def test_excel_to_database():
    ...
    conn, cursor = main.open_db("project_test_db")
    cursor.execute('DROP TABLE IF EXISTS Employment_Data')
    main.create_employment_data(cursor)
    main.excel_to_database(excel_data_path, cursor)
    # after creating and opening the database, run sql query to check we have 50 states
